#!/usr/bin/env bash
#
# pefile/mayhem/build.sh — build the Atheris fuzz target for erocarrera/pefile.
#
# This is a PYTHON (Atheris/libFuzzer) project, so the "build" is:
#   1) install pefile + atheris, OFFLINE, from the wheelhouse the Dockerfile baked into
...
#        - /mayhem/fuzz_exe             : the Mayhem libFuzzer target (Atheris iterates).
#        - /mayhem/fuzz_exe-standalone  : run-once reproducer (Atheris replays one file arg).
#        - /mayhem/run-cli             : the oracle runner mayhem/test.sh drives (show_exe.py).
#
...
# of which pefile has none. We still thread $DEBUG_FLAGS into the launcher compile so the spec's
# debug-info contract (DWARF < 4) holds on every emitted ELF.
set -euo pipefail
...
# atheris is the fuzzing engine; hatchling is pefile's PEP-517 build backend. pefile itself has no
# runtime dependencies (pure Python), so nothing else is needed for the parse path we fuzz.
PIP="python3 -m pip install --user --break-system-packages"
if [ -d "$WHEELHOUSE" ]; then
  $PIP --no-index --find-links "$WHEELHOUSE" atheris hatchling
  ...
  $PIP atheris hatchling
  $PIP --no-build-isolation .
fi

# Sanity: the harnessed module must import.
python3 -c 'import atheris, pefile; from pefile import PEFormatError' \
  || { echo "FATAL: pefile failed to import" >&2; exit 1; }
...
"$CC" $DEBUG_FLAGS -O1 \
    -DHARNESS_PATH="\"$SRC/mayhem/fuzz_exe.py\"" \
    -o "$OUT/fuzz_exe" "$SRC/mayhem/launcher.c"

# Standalone run-once reproducer: same binary (Atheris replays a single file argument).
cp -f "$OUT/fuzz_exe" "$OUT/fuzz_exe-standalone"

# CLI runner for the test oracle: exec's show_exe.py. Because it lives at a NON-system path, the
...
"$CC" $DEBUG_FLAGS -O1 \
    -DHARNESS_PATH="\"$SRC/mayhem/show_exe.py\"" \
    -o "$OUT/run-cli" "$SRC/mayhem/launcher.c"

echo "build.sh complete:"
ls -la "$OUT/fuzz_exe" "$OUT/fuzz_exe-standalone" "$OUT/run-cli"
